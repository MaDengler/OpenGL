...
void Shader::setColorUniformLocation(const char* name_uniform_variable){
	colorUniformLocation = glGetUniformLocation(shaderId, name_uniform_variable);
	if(!colorUniformLocation == -1)
		std::cout << "Color uniform could not be set" << std::endl;	
}

void Shader::setColorUniformValues(float r , float g, float b, float a){
	glUniform4f(colorUniformLocation, r, g, b, a);
}

void Shader::setTextureUniformLocation(const char* name_texture_uniform_variable){
	textureUniformLocation = glGetUniformLocation(shaderId, "u_texture");
	if(!textureUniformLocation == -1)
		std::cout << "Texture uniform could not be set" << std::endl;
}

void Shader::setTextureSlot(int slot){
